      ...
      <div
        ...
        className={`border-2 border-dashed rounded-lg p-8 text-center cursor-pointer transition-colors ${
          isDragActive
            ...
            : 'border-gray-300 dark:border-gray-600 hover:border-gray-400 hover:bg-gray-50'
      ...
      {uploads.length > 0 && (
        <div className="space-y-2">
          {uploads.map((u, i) => (
            <div
              key={i}
              className="flex items-center gap-3 p-2 bg-white dark:bg-gray-900 border rounded-md text-sm"
            >
              <span className="flex-1 truncate text-gray-700 dark:text-gray-300">{u.file.name}</span>
              <span className="text-gray-400 dark:text-gray-500">{formatFileSize(u.file.size)}</span>
              {u.status === 'uploading' && (
