  ...
  if (error) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-red-600 font-medium">{t('ui:TrashView.failedToLoadTrash')}</p>
      </div>
    )
  }
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100">{t('ui:TrashView.trash')}</h2>
          <span className="text-sm text-gray-500 dark:text-gray-400">({items.length} {t('ui:TrashView.items')}</span>
        </div>
        {items.length > 0 && (
          <button
            onClick={() => {
              if (confirm(t('ui:TrashView.permanentlyDeleteAllItemsIn'))) {
          ...
          >
            {emptyTrashMutation.isPending ? t('ui:TrashView.emptying') : t('ui:TrashView.emptyTrash')}
          </button>
        )}
      </div>
      ...
      {items.length === 0 ? (
        <div className="text-center py-16">
          ...
          <h3 className="text-gray-500 dark:text-gray-400 font-medium">{t('ui:TrashView.trashIsEmpty')}</h3>
          <p className="text-sm text-gray-400 dark:text-gray-500 mt-1">{t('ui:TrashView.itemsMovedToTrashWill')}</p>
        </div>
      ) : (
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center gap-1 shrink-0">
                  ...
                  <button
                    onClick={() => {
                      if (confirm(t('ui:TrashView.permanentlyDeleteV0ThisCannot', { v0: item.title || item.original_filename }))) {
                        deletePermanentlyMutation.mutate(item.id)
                      }
                    }}
                    disabled={deletePermanentlyMutation.isPending}
                    title={t('ui:TrashView.deletePermanently')}
