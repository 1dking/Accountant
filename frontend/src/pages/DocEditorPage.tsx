...
const TEXT_COLORS = [
  { label: i18n.t('ui:DocEditorPage.black'), value: '#000000' },
  { label: i18n.t('ui:DocEditorPage.red'), value: '#dc2626' },
  { label: i18n.t('ui:DocEditorPage.blue'), value: '#2563eb' },
  { label: i18n.t('ui:DocEditorPage.green'), value: '#16a34a' },
  { label: i18n.t('ui:DocEditorPage.purple'), value: '#9333ea' },
  { label: i18n.t('ui:DocEditorPage.orange'), value: '#ea580c' },
]
...
export default function DocEditorPage() {
  ...
  const editor = useEditor({
    extensions: [
      ...
      Highlight,
      Image,
      Placeholder.configure({ placeholder: t('ui:DocEditorPage.startTyping') }),
  ...
  const exportDocx = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(t('ui:DocEditorPage.exportFailed'))
  ...
  const handleImport = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(t('ui:DocEditorPage.importFailed'))
      // Refresh doc content
      queryClient.invalidateQueries({ queryKey: ['office-doc', id] })
      // Reset file input
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-49px)] bg-gray-100 dark:bg-gray-800">
      ...
      <EditorTopBar
        docType="document"
        docId={id}
        title={doc?.title || t('ui:DocEditorPage.untitledDocument')}
      ...
      <div className="bg-white dark:bg-gray-900 border-b px-3 py-1.5 flex items-center gap-2 flex-wrap">
          ...
          {showColorMenu && (
            <div className="absolute top-full left-0 mt-1 bg-white dark:bg-gray-800 border dark:border-gray-700 rounded-md shadow-lg z-50 py-1 min-w-[120px]">
              ...
              <div className="border-t dark:border-gray-700 mt-1 pt-1">
                ...
                >
                 {t('ui:DocEditorPage.removeColor')}
                </button>
              </div>
            </div>
          )}
        ...
        <div className="relative" ref={downloadMenuRef}>
          ...
          {showDownloadMenu && (
            <div className="absolute top-full left-0 mt-1 bg-white dark:bg-gray-800 border dark:border-gray-700 rounded-md shadow-lg z-50 py-1 min-w-[140px]">
              ...
              >
               {t('ui:DocEditorPage.docxDocx')}
              </button>
              <button
                onClick={exportPdf}
                className="block w-full text-left px-3 py-1.5 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700"
              >
               {t('ui:DocEditorPage.pdfPdf')}
              </button>
              <button
                onClick={exportMarkdown}
                className="block w-full text-left px-3 py-1.5 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700"
              >
               {t('ui:DocEditorPage.markdownMd')}
              </button>
            </div>
          )}
        </div>
        ...
        <button
          ...
          title={t('ui:DocEditorPage.findReplaceCtrlH')}
        >
          <Search className="h-3.5 w-3.5" />
         {t('ui:DocEditorPage.find')}
        </button>
        ...
        <button
          ...
          className="flex items-center gap-1 px-2 py-1 text-sm rounded-md border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors"
          title={t('ui:DocEditorPage.askAi')}
        >
          <Sparkles className="h-3.5 w-3.5 text-blue-500" />
         {t('ui:DocEditorPage.askAi')}
        </button>
        ...
        <button
          ...
          title={t('ui:DocEditorPage.toggleComments')}
        >
          <MessageSquare className="h-3.5 w-3.5" />
         {t('ui:DocEditorPage.comments')}
        </button>
        ...
        <button
          ...
          title={t('ui:DocEditorPage.toggleVersionHistory')}
        >
          <History className="h-3.5 w-3.5" />
         {t('ui:DocEditorPage.history')}
        </button>
        ...
        <button
          ...
          title={t('ui:DocEditorPage.toggleTableOfContents')}
        >
      ...
      {findReplaceOpen && (
        <div className="bg-white dark:bg-gray-900 border-b px-4 py-2 flex items-center gap-2 flex-wrap shadow-sm">
          ...
          <input
            type="text"
            placeholder={t('ui:DocEditorPage.find_2')}
            value={findText}
          ...
          <input
            type="text"
            placeholder={t('ui:DocEditorPage.replaceWith')}
            value={replaceText}
          ...
          <button
            ...
            className="px-2 py-1 text-xs font-medium rounded bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-600"
          >
           {t('ui:DocEditorPage.replace')}
          </button>
          <button
            onClick={handleReplaceAll}
            className="px-2 py-1 text-xs font-medium rounded bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-600"
          >
           {t('ui:DocEditorPage.replaceAll')}
          </button>
          {findResults.total > 0 && (
      ...
      <div className="bg-white dark:bg-gray-900 border-t dark:border-gray-700 px-4 py-1.5 flex items-center gap-4 text-xs text-gray-500 dark:text-gray-400">
        ...
        {/* Save Status */}
        <span>
          {updateMutation.isPending ? t('ui:DocEditorPage.saving') : t('ui:DocEditorPage.saved')}
        </span>
        ...
        <div className="flex items-center gap-1">
          <button
            ...
            title={t('ui:DocEditorPage.zoomOut')}
          >
          ...
          <button
            ...
            disabled={zoom >= 150}
            className="p-1 rounded hover:bg-gray-100 dark:hover:bg-gray-700 disabled:opacity-40 disabled:cursor-not-allowed"
            title={t('ui:DocEditorPage.zoomIn')}
          >
