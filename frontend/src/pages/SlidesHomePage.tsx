...
export default function SlidesHomePage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['office-docs', 'presentation', activeTab, debouncedSearch],
    queryFn: () =>
      listOfficeDocs({
  ...
  const createMutation = useMutation({
    mutationFn: (title: string) =>
      createOfficeDoc({ title: title || undefined, doc_type: 'presentation' }),
    onSuccess: (res) => {
  ...
  const handleTemplateClick = (template: typeof TEMPLATES[0]) => {
    const title = template.title === 'Blank' ? 'Untitled presentation' : template.title
              ...
              {TEMPLATES.map((template) => {
                ...
                return (
                  <button
                    ...
                    disabled={createMutation.isPending}
                    className="group flex flex-col items-center gap-2 p-4 rounded-lg border-2 border-dashed border-gray-200 dark:border-gray-700 hover:border-orange-400 hover:bg-orange-50/50 dark:hover:bg-orange-900/20 transition-colors w-32 disabled:opacity-50"
                  >
                    <div className="h-16 w-full bg-orange-50 dark:bg-orange-900/30 rounded flex items-center justify-center group-hover:bg-orange-100 dark:group-hover:bg-orange-900/50 transition-colors">
                      <Icon className="h-8 w-8 text-orange-400 group-hover:text-orange-600" />
                    </div>
