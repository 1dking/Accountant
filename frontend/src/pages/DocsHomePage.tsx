...
export default function DocsHomePage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['office-docs', 'document', activeTab, debouncedSearch],
    queryFn: () =>
      listOfficeDocs({
        doc_type: 'document',
        view: activeTab,
              ...
              {TEMPLATES.map((template) => {
                ...
                return (
                  <button
                    ...
                    disabled={createMutation.isPending}
                    className="group flex flex-col items-center gap-2 p-4 rounded-lg border-2 border-dashed border-gray-200 dark:border-gray-700 hover:border-blue-400 hover:bg-blue-50/50 dark:hover:bg-blue-900/20 transition-colors w-32 disabled:opacity-50"
                  >
                    <div className="h-16 w-full bg-blue-50 dark:bg-blue-900/30 rounded flex items-center justify-center group-hover:bg-blue-100 dark:group-hover:bg-blue-900/50 transition-colors">
                      <Icon className="h-8 w-8 text-blue-400 group-hover:text-blue-600" />
                    </div>
